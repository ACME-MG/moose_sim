...
def quick_ipf(exp_trajectories:list, sim_trajectories:list, file_path:str,
    ...
    """
    Quickly plots the experimental and simulated trajectories with
    predefined formatting

    Parameters:
    * `exp_trajectories`: List of experimental reorientation trajectories
    * `sim_trajectories`: List of simulated reorientation trajectories
    * `file_path`:        Path to save the IPF plot
    * `structure`:        The crystal structure
    * `direction`:        Direction to plot the IPF
    * `initial_only`:     Whether to just plot the initial orientations
    * `grain_ids`:        Experimental grain IDs; won't display if undefined
    """
